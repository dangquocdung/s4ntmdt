...
<body>
  <div class="wrapper">
    ...
    <input type="hidden" name="cart_url" id="cart_url" value="<?php echo e(route('cart-page')); ?>">
    <input type="hidden" name="currency_symbol" id="currency_symbol" value="<?php echo e($_currency_symbol); ?>">
    ...
    <?php if(Request::is('product/customize/*') || Request::is('product/details/*')): ?>
      <?php if(get_product_type($single_product_details['id']) == 'configurable_product' || get_product_type($single_product_details['id']) == 'customizable_product' || get_product_type($single_product_details['id']) == 'downloadable_product'): ?>
        <input type="hidden" name="selected_variation_id" id="selected_variation_id">
      <?php endif; ?>
    <?php endif; ?>
    
    <?php if(Request::is('checkout') || Request::is('cart')): ?>
      <div class="modal fade" id="customizeImages" tabindex="-1" role="dialog" aria-labelledby="updater" aria-hidden="true">
        <div class="modal-dialog">
          <div class="modal-content">  
            <div class="modal-header">
              <p class="no-margin"><?php echo trans('frontend.all_design_images'); ?></p>
    ...
    <?php echo $__env->make('modal.quick-view', array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>
  ...
  <input type="hidden" name="lang_code" id="lang_code" value="<?php echo e($selected_lang_code); ?>">  
  <input type="hidden" name="subscription_type" id="subscription_type" value="<?php echo e($subscriptions_data['subscribe_type']); ?>">
  
  <style type="text/css">
    #subscriptions_modal .modal__window{
      background: #<?php echo $subscriptions_data['popup_bg_color']; ?> !important;    
    }
  </style>
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function(){
      ...
      if($('#subscriptions_modal').length>0){
        <?php if($subscriptions_data['subscription_visibility'] == true && !$is_subscribe_cookie_exists && (!empty(get_current_page_name()) && is_array($subscriptions_data['popup_display_page']) && count($subscriptions_data['popup_display_page']) > 0 && in_array(get_current_page_name(), $subscriptions_data['popup_display_page']))): ?>
