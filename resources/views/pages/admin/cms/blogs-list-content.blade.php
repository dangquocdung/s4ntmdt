...
<div class="row">
  ...
  <div class="col-6">
    <div class="pull-right">
      <a href="{{ route('admin.add_blog') }}" class="btn btn-primary btn-sm">{!! trans('admin.add_new_post') !!}</a>
    </div>  
  </div>
</div>
<br>
              ...
              @foreach($blogs_list_data as $row)
                <tr>
                  @if(!empty($row['featured_image']))
                    <td><img src="{{ get_image_url( $row['featured_image'] ) }}" alt="{{ basename ($row['featured_image']) }}"></td>
                  @else
