function isValidEmail(a) { return /^[a-z0-9]+([-._][a-z0-9]+)*@([a-z0-9]+(-[a-z0-9]+)*\.)+[a-z]{2,4}$/.test(a) && /^(?=.{1,64}@.{4,64}$)(?=.{6,100}$).*/.test(a) }
var frontendLocalizationString, RGBChange = function() { $("#RGB").css("background", "rgb(" + r.getValue() + "," + g.getValue() + "," + b.getValue() + ")") };
$(document).ready(function() { $(function() { $.scrollUp({ scrollName: "scrollUp", scrollDistance: 300, scrollFrom: "top", scrollSpeed: 300, easingType: "linear", animation: "fade", animationSpeed: 200, scrollTrigger: !1, scrollText: '<i class="fa fa-angle-up"></i>', scrollTitle: !1, scrollImg: !1, activeOverlay: !1, zIndex: 2147483647 }) }), eBazar_frontend.init.pageLoad(), $("#productRequest").on("hidden.bs.modal", function() { $(".subscribe-message").text(""), $("#subscribe_email").val("") }), $("#frontend_all_msg_with_localization").length > 0 && (frontendLocalizationString = JSON.parse($("#frontend_all_msg_with_localization").val())) });
var eBazar_frontend = eBazar_frontend || {};
eBazar_frontend.init = { pageLoad: function() { $(".category-products .collapse").on("shown.bs.collapse", function() { $(this).parent().find(".fa-plus").removeClass("fa-plus").addClass("fa-minus") }).on("hidden.bs.collapse", function() { $(this).parent().find(".fa-minus").removeClass("fa-minus").addClass("fa-plus") }), eBazar_frontend.event.filterProducts(), jQuery("#productVideoDisplay iframe").length > 0 && (jQuery("#productVideoDisplay iframe").removeAttr("width"), jQuery("#productVideoDisplay iframe").removeAttr("height"), $("#productVideoDisplay").on("hidden.bs.modal", function() { var a = $("#productVideoDisplay iframe").attr("src");
            $("#productVideoDisplay iframe").attr("src", ""), $("#productVideoDisplay iframe").attr("src", a) })), jQuery("#productVideoDisplay video").length > 0 && $("#productVideoDisplay").on("hidden.bs.modal", function() { var a = document.getElementById("product_video");
