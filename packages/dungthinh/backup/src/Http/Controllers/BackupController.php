...
class BackupController extends Controller
{
    ...
    public function postCreate(CreateBackupRequest $request)
    {
        try {
            ...
            return [
                'error' => false,
                'message' => __('Tạo bản sao lưu thành công!'),
    ...
    public function postDeleteMany(Request $request)
    {
        ...
        return [
            'error' => false,
            'message' => __('Xoá nhiều mục chọn thành công!'),
            'data' => $this->getBackupListHtml(),
        ];
    }
    ...
    public function postRestore($key)
    {
        ...
        foreach (scan_folder($path) as $file) {
            ...
            if (str_contains(basename($file), 'public-storage')) {
                $this->backup->restore($path . DIRECTORY_SEPARATOR . $file, public_path());
            }
        }

        return [
            'error' => false,
            'message' => __('Khôi phục thành công!'),
        ];
    }
